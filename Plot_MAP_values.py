...
Main Plot results on a map: circle are more r less big as function of the values 
"""
import h5py
import numpy as np
...
outfile3 = 'MAP_EQ_4_7distmax_200_Mean_peaktoPeak'
...
for i in h:
    ...
    for st in k :
        ...
        for Component in ['Z','R','T','E','N','H'] : 
            ...
            if f56[i][st]['SNR_{}'.format(Component)][0]==0:
...
for i in h:
    ...
    if len(k)<>7 :
        ...
        if isnan(a[0])==False :
            size = int(np.mean(np.array(f56[i]['St3']['PtP_{}'.format('E')][0:3])/np.array(f56[i]['St5']['PtP_{}'.format('E')][0:3])))*20+40
            Lsize.append(size)
            Lvalues.append(np.mean(np.array(f56[i]['St3']['PtP_{}'.format('E')][0:3])/np.array(f56[i]['St5']['PtP_{}'.format('E')][0:3])))
...
f67 = h5py.File('/home/claire/PHD/Working/Results/Peak_to_Peak/Parallel_PeaktoPeak_test2_V2_Ml_%s_%s_distmax_%s.hdf5'%(str(MlMin),str(MlMax),str(DistMax)),'r+')
g= f67.keys()
h=copy.copy(g)
for i in g:
    k=f67[i].keys()
    ...
    for st in k :
        ...
        for Component in ['Z','R','T','E','N','H'] : 
            ...
            if f67[i][st]['SNR_{}'.format(Component)][0]==0:
                print 'siganl satured or SNR to low', i , Component,st
                if i in h : 
                    h.remove(i)
for i in h:
    ...
    if len(k)<>7 :
        ...
        MLcolor.append(DCOLORS[col])
        size = int(np.mean(np.array(f67[i]['St3']['PtP_{}'.format('E')][0:3])/np.array(f67[i]['St5']['PtP_{}'.format('E')][0:3])))*10+10
        Lsize.append(size)
        Lvalues.append(np.mean(np.array(f67[i]['St3']['PtP_{}'.format('E')][0:3])/np.array(f67[i]['St5']['PtP_{}'.format('E')][0:3])))
